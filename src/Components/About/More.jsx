...
const AnimatedScrollElement = () => {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(handleIntersection, {
      threshold: 0.5 // Cuando al menos el 50% del elemento esté visible
    });
  ...
  useEffect(() => {
    if (isIntersecting) {
      animatedElementRef.current.classList.add('animate');
    }
  }, [isIntersecting]);
  ...
  return (
    <div className="container bg-white">
      ...
      <div className="animated-element" ref={animatedElementRef}>Elemento animado</div>
      <div className="spacer" style={{ height: '300px' }}></div>
    </div>
  );
};
