    ...
    <nav className={`fixed w-full top-0 left-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-md' : 'bg-black bg-opacity-50'}`}>
      ...
      <div className={`${isOpen ? 'block' : 'hidden'} md:hidden  `}>
        <div className="px-2 pt-2 pb-2 space-y-1 sm:px-3">
          {LinkItems.map(l => {
            return <Link key={l.link} className={`border-b border-solid border-grey  ${isScrolled ? 'text-black' : 'text-white' } hover:text-orange block px-1 mx-6 py-7 text-center text-base font-medium`} href={l.linkRef}>{l.link}</Link>
          }
          )}
        </div>
      </div>
