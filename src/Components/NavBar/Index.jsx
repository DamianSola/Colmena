'use client';
import logo from "./ColmenaLogo.png"
...
const Navbar = () => {
    ...
    return (
      <nav className=" bg-white dark:bg-darkgrey  fixed w-full z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 sticky z-auto">
          <div className="flex items-center justify-between h-16">
            <div className="flex-shrink-0">
              {/* <h1 className="text-black">Colmena</h1> */}
              <Image src={logo} width='150'  alt="colmena-logo"/>
            </div>
            <div className="hidden md:block">
              <div className="ml-10 flex items-baseline space-x-4">
              {LinkItems.map(l => {
                    return <Link key={l.link} className="text-black dark:text-white hover:text-orange px-5 py-2 rounded-md text-base font-medium" href={l.linkRef} >{l.link}</Link>
                }
                )}
              </div>
            </div>
